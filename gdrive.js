...
const GDriveSync = {
    ...
    async findFileId(fileName) {
        try {
            GDriveSync.log(`Searching for file ID of ${fileName}...`);
            const response = await fetch(`https://www.googleapis.com/drive/v3/files?q=name='${fileName}' and trashed=false&fields=files(id)`, {
                headers: {
                    "Authorization": "Bearer " + GDriveSync.accessToken
                }
            });
    ...
    async findFileIdPublic(fileName) {
        ...
        try {
            const response = await fetch(`https://www.googleapis.com/drive/v3/files?q=name='${fileName}' and trashed=false&fields=files(id)&key=${GDriveSync.apiKey}`);
